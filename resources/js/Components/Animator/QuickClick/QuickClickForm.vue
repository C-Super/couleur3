...
const submit = () => {
    axios.post(route("interactions.quick_click.store"), form).then((response) => {
...
<template>
    <div>
        <!-- Dashboard card -->
        <base-card v-if="!data.creatingInteraction && !data.currentInteraction" :color="Color.ACCENT">
        ...
        <!-- Create form -->
        <form v-if="data.creatingInteraction === InteractionType.QUICK_CLICK" @submit.prevent="submit">
        <base-card :color="Color.ACCENT">
            ...
            <template #subtitle>
             ...
             <template #content>
                <input-group id="title" label="Titre">
                    <text-input id="title" v-model="form.title" :color="Color.ACCENT" />
                </input-group>
            ...
            <template #actions>
                <div class="flex flex-row gap-3">
                    <base-button @click="cancelQuickClick">Annuler</base-button>
        ...
        <!-- Result pages -->
        <base-card v-if="data.currentInteraction && data.currentInteraction.type === InteractionType.QUICK_CLICK" :color="Color.ACCENT">
