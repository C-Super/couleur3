<template>
    <div class="flex">
        ...
        <div class="ml-4 grow flex flex-col justify-center gap-y-1">
            <p id="emission_title" class="text-base leading-5">
                La chose publique
            </p>
            <p class="text-xs font-light">
                Présenté par
                <span class="text-primary">Patrick&nbsp;Dujany</span>
            </p>
        </div>
        <button class="shrink-0 px-4" @click="toggleVideo">
            <span v-if="!isPlaying" class="material-symbols-rounded text-4xl">
                play_arrow
            </span>
            <span v-else class="material-symbols-rounded text-4xl">pause</span>
        </button>
    </div>
</template>
